...
import { getDailyTrackers } from 'apiSdk/daily-trackers';
import { DailyTrackerInterface } from 'interfaces/daily-tracker';
import { getPartPayments } from 'apiSdk/part-payments';
import { PartPaymentInterface } from 'interfaces/part-payment';
import { getLoanDetails } from 'apiSdk/loan-details';
import { LoanDetailsInterface } from 'interfaces/loan-details';

import { LoanClosureListPage as ShowCaseList } from 'pages/loan-closures';

function DashboardPage() {
  const { session } = useSession();

  const dailyTrackerStat = useSWR<PaginatedInterface<DailyTrackerInterface>>(
    () => `/daily-trackers`,
    () => getDailyTrackers({ limit: 1, offset: 0 }),
  );

  const partPaymentStat = useSWR<PaginatedInterface<PartPaymentInterface>>(
    () => `/part-payments`,
    () => getPartPayments({ limit: 1, offset: 0 }),
  );

  const loanDetailsStat = useSWR<PaginatedInterface<LoanDetailsInterface>>(
    () => `/loan-details`,
    () => getLoanDetails({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Daily Trackers',
      result: dailyTrackerStat?.data?.totalCount,
      isLoading: dailyTrackerStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Part Payments',
      result: partPaymentStat?.data?.totalCount,
      isLoading: partPaymentStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Loan Details',
      result: loanDetailsStat?.data?.totalCount,
      isLoading: loanDetailsStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Loan Closures
                </Text>
                <NextLink href={`loan-closures`} passHref legacyBehavior>
                  <Button
